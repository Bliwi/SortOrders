...
class sortorders_maintain extends PluginMaintain
{
  ...
  function __construct($plugin_id)
  {
    ...
    $this->table = $prefixeTable . 'sortorders';
    $this->dir = PHPWG_ROOT_PATH . PWG_LOCAL_DIR . 'sortorders/';
  }
  ...
  function install($plugin_version, &$errors=array())
  {
    ...
    if (empty($conf['sortorders']))
    {
      ...
      // the third parameter indicates to update $conf['sortorders'] global variable as well
...
;');
    ...
    $result = pwg_query('SHOW COLUMNS FROM `'.IMAGES_TABLE.'` LIKE "sortorders";');
    if (!pwg_db_num_rows($result))
    {
      pwg_query('ALTER TABLE `' . IMAGES_TABLE . '` ADD `sortorders` TINYINT(1) NOT NULL DEFAULT 0;');
    }
  ...
  function uninstall()
  {
    ...
    // delete field
    pwg_query('ALTER TABLE `'. IMAGES_TABLE .'` DROP `sortorders`;');
